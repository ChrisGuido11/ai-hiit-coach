...
interface TimerState {
  ...
  mainTimeRemaining: number; // Only used in main phase - countdown timer
  currentExerciseTime: number; // Time remaining for current exercise in warmup/cooldown
  isPaused: boolean;
}
...
// AMRAP uses time-based duration (seconds)
const WARMUP_ROUNDS = 2; // Warmup cycles through exercises
const COOLDOWN_ROUNDS = 1; // Cooldown cycles through exercises
// MAIN_SECONDS is calculated from workout duration - default 1200 seconds (20 minutes)
const DEFAULT_MAIN_SECONDS = 1200;

// Phase colors (AMRAP uses consistent colors - no work/rest switching)
const phaseColors = {
  warmup: {
    primary: "#FF9500",
...
// Calculate total AMRAP main duration in seconds based on number of exercises
// Allocate time proportionally: warmup ~3min, main ~proportional, cooldown ~3min
const calculateAMRAPSeconds = (mainExerciseCount: number): number => {
  // Default calculation: more exercises = longer AMRAP duration
  // Minimum 15 minutes, maximum 30 minutes for main phase (900-1800 seconds)
  // Formula: 60 seconds per exercise
  const baseSeconds = Math.max(900, Math.min(1800, mainExerciseCount * 60));
  return baseSeconds;
};
...
const AMRAPTimer = () => {
  ...
  const [timerState, setTimerState] = useState<TimerState>({
    ...
    exerciseIndex: 0,
    roundsCompleted: 0,
    mainTimeRemaining: 0, // Will be initialized by useEffect
    currentExerciseTime: 0, // Will be initialized by useEffect
    isPaused: true, // Start paused until initialized
  });
  const [isInitialized, setIsInitialized] = useState(false);
  const [totalMainSeconds, setTotalMainSeconds] = useState(DEFAULT_MAIN_SECONDS);
  ...
  // Calculate total AMRAP seconds based on number of exercises
  useEffect(() => {
    if (typedWorkout?.main && typedWorkout.main.length > 0) {
      const calculatedSeconds = calculateAMRAPSeconds(typedWorkout.main.length);
      setTotalMainSeconds(calculatedSeconds);
    }
  ...
  useEffect(() => {
    ...
    if (warmupExercises.length > 0) {
      ...
      setTimerState({
        ...
        exerciseIndex: 0,
        roundsCompleted: 0,
        mainTimeRemaining: totalMainSeconds,
  ...
  }, [typedWorkout, isInitialized, totalMainSeconds]);
  ...
  // Get max rounds for current phase (AMRAP main phase uses time, not rounds)
  const getMaxRounds = useCallback((): number => {
    switch (timerState.phase) {
      case "warmup":
        return WARMUP_ROUNDS;
      case "main":
        return 999; // AMRAP doesn't have a round limit, just time limit
      case "cooldown":
  ...
  // Get next exercise in the rotation (cycles continuously for AMRAP)
  const getNextExerciseInRotation = useCallback((): { exercise: Exercise | null; isNextRound: boolean; isNextPhase: boolean } => {
    ...
    if (nextIndex < exercises.length) {
      ...
      // Finished all exercises in this round, start next round (AMRAP specific)
      if (timerState.phase === "main") {
        // In AMRAP main, always cycle back to first exercise (time will end it)
        return { exercise: exercises[0], isNextRound: true, isNextPhase: false };
      } else {
        // Warmup/Cooldown: check if we've completed all rounds
        if (timerState.roundsCompleted + 1 < maxRounds) {
          return { exercise: exercises[0], isNextRound: true, isNextPhase: false };
        } else {
  ...
  }, [getCurrentExercises, timerState.exerciseIndex, timerState.phase, timerState.roundsCompleted, maxRounds]);
  ...
  // Get total duration for current interval (used only for progress visualization)
  const getTotalDuration = useCallback((): number => {
    if (timerState.phase === "main") {
      return totalMainSeconds; // AMRAP uses total time for main phase
    }
    if (currentExercise) {
      return parseDuration(currentExercise.duration);
    }
    return 45;
  }, [timerState.phase, currentExercise, totalMainSeconds]);

  // Calculate progress (0 to 1) - AMRAP main uses mainTimeRemaining
  const progress = timerState.phase === "main"
    ? timerState.mainTimeRemaining / getTotalDuration()
    : timerState.currentExerciseTime / getTotalDuration();
  ...
  // Move to next state - AMRAP version: continuous cycling with time countdown
  const advanceTimer = useCallback(() => {
    setTimerState((prev) => {
      const phaseMaxRounds = prev.phase === "warmup" ? WARMUP_ROUNDS
        : prev.phase === "main" ? 999 // AMRAP doesn't have round limit
        : COOLDOWN_ROUNDS;

      // Main phase logic (AMRAP: continuous exercise cycling with time countdown)
      if (prev.phase === "main") {
        const mainExercises = typedWorkout?.main || [];
        ...
        let nextRoundsCompleted = prev.roundsCompleted;

        // Check if we completed a full round
        if (nextExerciseIndex >= mainExercises.length) {
          nextRoundsCompleted = prev.roundsCompleted + 1;
        }

        // Always cycle to next exercise (AMRAP runs until time runs out)
        ...
        // Note: mainTimeRemaining will be decremented by the timer tick
        // We just advance the exercise here
        return {
          ...prev,
      ...
      if (nextExerciseIndex < exercises.length) {
        ...
        if (prev.roundsCompleted + 1 < phaseMaxRounds) {
          ...
          if (needsSideSwitch) {
            const bodyPart = getBodyPartTerm(firstExercise);
            const sideText = getSideAnnouncement("right", bodyPart);
            speak(`Round ${nextRounds + 1}! ${firstExercise.name}, ${sideText}`, true);
          } else {
            speak(`Round ${nextRounds + 1}! ${firstExercise.name}`, true);
          }
  ...
  }, [typedWorkout, speak, vibrate, startPhaseTransition, totalMainSeconds]);
      ...
      setTransition((prev) => {
        ...
        if (prev.countdown <= 1) {
          ...
          if (prev.type === "phase" && prev.nextPhase) {
            ...
            let mainDuration = totalMainSeconds;
            let exerciseDuration = 0;

            if (prev.nextPhase === "main") {
              mainDuration = totalMainSeconds;
              ...
              if (cooldownExercise) {
                ...
                exerciseDuration = match ? parseInt(match[1], 10) : 45;
              }
            }
            ...
            setTimerState((ts) => ({
              ...
              exerciseIndex: 0,
              roundsCompleted: 0,
              mainTimeRemaining: prev.nextPhase === "main" ? mainDuration : ts.mainTimeRemaining,
              currentExerciseTime: prev.nextPhase === "main" ? 0 : exerciseDuration,
            }));
  ...
  useEffect(() => {
    ...
    intervalRef.current = setInterval(() => {
      setTimerState((prev) => {
        ...
        if (prev.phase === "main") {
          // AMRAP main phase: countdown mainTimeRemaining
          if (prev.mainTimeRemaining <= 0) {
            return prev; // Timer ended, will trigger phase completion
          }

          const newMainTime = prev.mainTimeRemaining - 1;

          // Count down voice for last 3 seconds
          if (newMainTime <= 3 && newMainTime > 0) {
            speak(newMainTime.toString());
          }

          return { ...prev, mainTimeRemaining: newMainTime };
        } else {
          // Warmup/Cooldown: countdown currentExerciseTime
          if (prev.currentExerciseTime <= 0) {
            return prev; // Exercise ended, will trigger advance
          }

          const newExerciseTime = prev.currentExerciseTime - 1;

          // Count down voice for last 3 seconds
          if (newExerciseTime <= 3 && newExerciseTime > 0) {
            speak(newExerciseTime.toString());
          }

          return { ...prev, currentExerciseTime: newExerciseTime };
        }
      });
    }, 1000);
  ...
  // Check for timer completion - AMRAP uses two different timers
  useEffect(() => {
    if (timerState.phase === "complete" || transition) return;

    if (timerState.phase === "main") {
      // Main phase ends when mainTimeRemaining reaches 0
      if (timerState.mainTimeRemaining <= 0) {
        // Move to cooldown or complete
      ...
      // Warmup/Cooldown: advance when currentExerciseTime reaches 0
      if (timerState.currentExerciseTime <= 0) {
        advanceTimer();
      }
    }
  }, [timerState.mainTimeRemaining, timerState.currentExerciseTime, timerState.phase, transition, typedWorkout, advanceTimer, startPhaseTransition]);
  ...
  useEffect(() => {
    ...
    if (timerState.currentExerciseTime === halfwayPoint && halfwayPoint > 0) {
  ...
  }, [timerState.currentExerciseTime, timerState.phase, timerState.isPaused, transition, currentExercise, hasAnnouncedSwitch, speak, vibrate]);
  ...
  const handleRefreshConfirm = async () => {
    ...
    try {
      ...
      // while preserving phase, exerciseIndex, and roundsCompleted
      if (timerState.phase === 'main') {
        // Main workout: Keep time running (no reset for AMRAP)
        setTimerState(prev => ({
          ...prev,
          isPaused: false  // Resume timer automatically
        }));
      } else {
    ...
    try {
      ...
      if (timerState.phase === 'main') {
        ...
        setTimerState(prev => ({
          ...prev,
          isPaused: false  // Resume timer automatically
          // Note: phase, exerciseIndex, and roundsCompleted are preserved from prev
        }));
      } else {
        // Warmup/Cooldown: Parse duration from new exercise
        const match = newExercise.duration.match(/(\d+)/);
  ...
  // Get colors based on current phase (AMRAP doesn't switch between work/rest)
  const getColors = () => {
    ...
    return {
      ...
      text: timerState.phase === "main" ? "AMRAP" : timerState.phase.toUpperCase(),
    };
  };
      ...
      <div className="min-h-screen bg-[#0A1F2E] flex flex-col items-center justify-center p-6">
        ...
        <div className="text-center">
          ...
          <p className="text-[#B0B8C1] mb-8 fade-in-up" style={{ animationDelay: '0.1s' }}>
            Amazing work! You crushed that AMRAP workout. {timerState.roundsCompleted} rounds completed!
          </p>
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center z-20">
              ...
              >
                {timerState.phase === "main"
                  ...
                  : `Round ${timerState.roundsCompleted + 1} of ${maxRounds}`}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="text-center h-14 flex items-center justify-center">
          {!transition && (() => {
            ...
            if (nextInfo.isNextRound) {
              ...
              return (
                <span className="text-sm text-slate-400">
                  Next: Round {timerState.roundsCompleted + 2} → {nextInfo.exercise?.name}
                </span>
              );
            }
